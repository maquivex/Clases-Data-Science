...
for pagina in range(1, total_paginas +1):
    ...
    if response.status_code == 200:
        ...
        for art in autos:
            ...
            cursor.execute(
                """
                insert into autos(titulo,link,etiqueta,imagen,combustible,ubicacion,precio,marca,anio,anunciante)
                values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                """,
                (title,link,tag,image,fuel,location,price,data["item_brand"],data["item_year"],data["item_advertiser"])
            )
            conn.commit()
